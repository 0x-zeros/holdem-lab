import json
from pathlib import Path
from typing import Any, cast

import cv2
import numpy as np
from holdem_bot.vision import (
    build_and_evaluate_poker_legends_card_classifier,
    build_and_evaluate_poker_legends_card_part_templates,
    evaluate_poker_legends_card_consensus_suite,
)


def test_card_consensus_generalizes_to_unseen_card_combinations(tmp_path: Path) -> None:
    annotation_dir = tmp_path / "annotations"
    image_root = tmp_path / "images"
    annotation_dir.mkdir()
    image_root.mkdir()
    truth_paths = []
    for card in ("AS", "AH", "KS", "KH"):
        frame_id = f"frame_{card}"
        write_synthetic_consensus_card_frame(
            image_root / f"{frame_id}.png",
            annotation_dir / f"{frame_id}.json",
            card=card,
        )
        truth_path = tmp_path / f"{frame_id}_truth.json"
        truth_path.write_text(
            json.dumps(
                {
                    "frame_id": frame_id,
                    "screen": {"kind": "actionable_table"},
                    "hero_hole_cards": [
                        {
                            "slot": "hero_hole_0",
                            "visible": True,
                            "card": card,
                            "confidence": 1.0,
                        }
                    ],
                    "board": [],
                    "ignored_fields": [],
                }
            ),
            encoding="utf-8",
        )
        truth_paths.append(truth_path)

    part_summary = build_and_evaluate_poker_legends_card_part_templates(
        truth_paths,
        annotation_dir=annotation_dir,
        image_root=image_root,
        output_dir=tmp_path / "parts",
        rank_max_distance=0.01,
        suit_max_distance=0.01,
    )
    classifier_summary = build_and_evaluate_poker_legends_card_classifier(
        truth_paths,
        annotation_dir=annotation_dir,
        image_root=image_root,
        output_dir=tmp_path / "classifier",
        visibility_max_distance=0.30,
        rank_max_distance=0.30,
        suit_max_distance=0.30,
    )
    summary = cast(
        dict[str, Any],
        evaluate_poker_legends_card_consensus_suite(
            truth_paths,
            annotation_dir=annotation_dir,
            image_root=image_root,
            output_dir=tmp_path / "consensus",
            card_part_manifest=tmp_path / "parts" / str(part_summary["manifest"]),
            card_classifier_manifest=tmp_path / "classifier" / str(classifier_summary["manifest"]),
        ),
    )

    assert summary["self_eval"]["visible_accuracy"] == 1.0
    assert summary["leave_frame_eval"]["visible_accuracy"] == 1.0
    assert summary["leave_card_eval"]["visible_accuracy"] == 1.0
    assert summary["leave_card_eval"]["hidden_false_positive_rate"] == 0.0
    assert (tmp_path / "consensus" / "card_consensus_report.md").exists()


def write_synthetic_consensus_card_frame(
    image_path: Path,
    annotation_path: Path,
    *,
    card: str,
) -> None:
    image = np.full((180, 220, 3), 32, dtype=np.uint8)
    image[20:156, 30:132] = 245
    image[20:156, 148:202] = 58
    color = (0, 0, 180) if card[1] == "H" else (0, 0, 0)
    cv2.putText(
        image,
        card[0],
        (42, 64),
        cv2.FONT_HERSHEY_SIMPLEX,
        1.35,
        color,
        4,
        cv2.LINE_AA,
    )
    cv2.putText(
        image,
        card[1],
        (45, 118),
        cv2.FONT_HERSHEY_SIMPLEX,
        1.35,
        color,
        4,
        cv2.LINE_AA,
    )
    cv2.imwrite(str(image_path), image)
    annotation_path.write_text(
        json.dumps(
            {
                "image": image_path.name,
                "regions": {
                    "cards": [
                        {
                            "name": "hero_hole_0",
                            "kind": "card",
                            "rect": {"x": 30, "y": 20, "width": 102, "height": 136},
                        }
                    ],
                    "board": [
                        {
                            "name": "board_0",
                            "kind": "card",
                            "rect": {"x": 148, "y": 20, "width": 54, "height": 136},
                        }
                    ],
                },
            }
        ),
        encoding="utf-8",
    )
