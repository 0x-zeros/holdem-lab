...
> **省 LLM 调用(CV 回合扳机)**:LLM 模式下每帧先用廉价 CV 检测动作按钮(`detect_action_buttons`,几毫秒),
> **只有检测到按钮 = 轮到 hero 时才触发昂贵的 LLM 读**,对手回合一律不调,非回合也不画陈旧靶标。扑克是回合制、
> 只需自己回合读一次,所以 LLM 延迟与游戏动画/对手速度解耦。HUD 顶行 `cv Nbtn hero_turn~ llm_reads` 显示扳机
> 状态与累计调用数。
